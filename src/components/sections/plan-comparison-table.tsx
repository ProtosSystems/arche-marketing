    ...
    <tbody>
      ...
      {group.features.map((feature) => (
        <tr key={String(feature.name)} className="group">
          <th
            scope="row"
            className="border-t border-mist-950/5 py-4 pr-3 font-normal text-slate-700 group-first:border-mist-950/10 dark:border-white/5 dark:text-slate-300 dark:group-first:border-white/10"
          >
            {feature.name}
          </th>
          {plans.map((plan) => {
            ...
            return (
              <td
                key={plan}
                className="border-t border-mist-950/5 px-3 py-4 text-center text-slate-700 group-first:border-mist-950/10 dark:border-white/10 dark:text-slate-300 dark:group-first:border-white/10"
              >
                {value === true ? (
            ...
            <ElTabList className="flex gap-6">
              {plans.map((plan) => (
                <button
                  key={plan}
                  type="button"
                  className="relative -mb-px flex-1 border-b border-b-transparent px-2 py-6 text-sm/5 font-medium text-slate-600 aria-selected:border-mist-950 aria-selected:text-mist-950 dark:aria-selected:border-white dark:aria-selected:text-white"
                >
                  {plan}
                </button>
              ))}
