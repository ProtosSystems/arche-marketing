...
export function FooterLink({ href, className, ...props }: { href: string } & Omit<ComponentProps<'a'>, 'href'>) {
  return (
    <li className={clsx('text-slate-700 dark:text-slate-300', className)}>
      <a href={href} {...props} />
    </li>
  )
}
...
export function NewsletterForm({
  ...
  return (
    <form className={clsx('flex max-w-sm flex-col gap-2', className)} {...props}>
      <p>{headline}</p>
      <div className="flex flex-col gap-4 text-slate-700 dark:text-slate-300">{subheadline}</div>
...
export function FooterWithNewsletterFormCategoriesAndSocialIcons({
  ...
  return (
    <footer className={clsx('pt-16', className)} {...props}>
      <div className="bg-mist-950/2.5 py-16 sm:py-20 text-mist-950 dark:bg-white/5 dark:text-white">
        <Container className="flex flex-col gap-16">
          {topCta && <div>{topCta}</div>}
          <div className="grid grid-cols-1 gap-x-6 gap-y-16 text-sm/7 lg:grid-cols-2">
            {cta}
            <div
              className="grid grid-cols-2 gap-6 sm:has-[>:last-child:nth-child(3)]:grid-cols-3 sm:has-[>:nth-child(5)]:grid-cols-3 md:has-[>:last-child:nth-child(4)]:grid-cols-4 lg:max-xl:has-[>:last-child:nth-child(4)]:grid-cols-2"
            >
              {links}
            </div>
          </div>
          <div className="flex items-center justify-between gap-10 text-sm/7">
            <div className="text-slate-600 dark:text-slate-400">{fineprint}</div>
